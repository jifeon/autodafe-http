...
//            this.log('304. File `%s` not modified ( session id=%s )'.format(filePath, this.get_session_id()));
        ...
        fs.readFile(filePath, "binary", this._sendFileContent.bind(this, filePath));
    },
     ...
     * @private
     */
    _sendFileContent: function (filePath, e, fileContent) {
        if (e) {
            // todo: 403 or 500?
...
//        this.log('Error %s by address `%s`'.format(e.number, this.request.url), 'warning');
        this._originResponse.statusCode = e.number;
...
//        }
//        catch (err) {
            this.end(s('<h1>Error %s. %s</h1>', e.number, http.STATUS_CODES[ e.number ] || ''), 'utf8');
//        }
    },
